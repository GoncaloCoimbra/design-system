...
const meta = {
  title: "Brand Identity/CommitPT Landing Page",
  parameters: {
    layout: "fullscreen",
    docs: {
      description: {
        component:
          "A full landing-page composition inspired by the CommitPT website, assembled from the design-system foundations and website-pattern stories.",
      },
    },
  },
  tags: ["autodocs"],
...
function Section({ children, className }: { children: React.ReactNode; className?: string }) {
  return (
    <section
      className={`mx-auto flex w-full max-w-7xl flex-col px-6 py-16 sm:px-10 lg:px-12 ${className ?? ""}`}
    >
      {children}
    </section>
  );
  ...
  render: () => (
    <div className="min-h-screen bg-background text-foreground">
      <Section className="pt-8">
        <div className="flex items-center justify-between rounded-full border border-border/70 bg-surface/70 px-4 py-3 backdrop-blur">
          <div className="flex items-center gap-3">
            <Avatar size="sm" variant="primary">
              <AvatarImage src={commitIcon} alt="CommitPT logo" />
              <AvatarFallback>CP</AvatarFallback>
            </Avatar>
            <Typography variant="large" className="text-sm font-semibold text-foreground">
              CommitPT
            </Typography>
          </div>
          ...
          <Button size="sm">Entrar agora</Button>
        </div>
      </Section>

      <Section>
        <div className="grid items-center gap-10 rounded-[32px] border border-border/70 bg-surface/70 p-8 shadow-[0_24px_80px_rgba(0,0,0,0.28)] lg:grid-cols-[1.15fr_0.85fr] lg:p-12">
      ...
      <Section className="pt-0">
        <div className="rounded-[28px] border border-primary/20 bg-gradient-to-br from-primary/10 via-background to-secondary/10 p-8 lg:p-10">
            ...
            <div className="flex flex-col gap-4">
              ...
              <div>
                ...
                <Typography variant="p" className="mt-2 text-muted-foreground">
                  Bruno Moisão criou a CommitPT porque sentiu que faltava um espaço onde
                  programadores pudessem aprender juntos, receber feedback e crescer de forma mais
                  prática.
                </Typography>
              </div>
            </div>
            <div className="flex flex-col justify-between gap-5 rounded-2xl border border-border/70 bg-background/70 p-6">
      ...
      <Section className="pt-0">
        <div className="rounded-[28px] border border-border/70 bg-surface/70 p-8 lg:p-10">
